...
public class EventAddForm extends SideMenuBaseForm {

    public EventAddForm(Resources res) {
        ...
        for (Salle s : ServiceSalle.getInstance().affichageSalle()) {
            salle.addItem(String.valueOf(s.getNbs()));
        }
        ...
        TextField nbp = new TextField("", "", 20, TextField.EMAILADDR);
        TextField typr = new TextField("", "", 20, TextField.USERNAME);
        TextField nom = new TextField("", "", 20, TextField.USERNAME);

        Button addButton = new Button("ADD");
        addButton.addActionListener(e -> {
            if (Dialog.show("Succes", "The Event will be added", "Ok", "Cancel")) {
                SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
                ServiceEvent.getInstance().ajouterEvent(new Event(nom.getText(),typr.getText(), Integer.parseInt(salle.getSelectedItem()),format.format(dated.getDate()), format.format(datef.getDate()),Integer.parseInt(nbp.getText())));
